...
function mapAbsolutePath(path: string): string {
  ...
  return absolutePath.length > 1 // solve https://github.com/spencerwooo/onedrive-vercel-index/issues/539
    ? absolutePath[1]
        .split('/')
        .map(p => encodeURIComponent(safeDecodeURIComponent(p)))
        .join('/')
    : ''
}
...
function SearchResultItemLoadRemote({ result }: { result: OdSearchResult[number] }) {
  const { data, error }: SWRResponse<OdDriveItem, { status: number; message: unknown }> = useSWR<
    OdDriveItem,
    { status: number; message: unknown }
  >([`/api/item/?id=${result.id}`], fetcher)

  if (error) {
    const errorMessage = (error.message as { error?: unknown } | null)?.error
    return (
      <SearchResultItemTemplate
        driveItem={result}
        driveItemPath={''}
        itemDescription={typeof errorMessage === 'string' ? errorMessage : JSON.stringify(error.message)}
        disabled={true}
      />
    )
  }
  ...
  return (
    <SearchResultItemTemplate
      ...
      itemDescription={safeDecodeURIComponent(driveItemPath)}
      disabled={false}
    />
  )
}
...
function SearchResultItem({ result }: { result: OdSearchResult[number] }) {
  if (result.path === '') {
    ...
    const driveItemPath = safeDecodeURIComponent(result.path)
    return (
      <SearchResultItemTemplate
        driveItem={result}
