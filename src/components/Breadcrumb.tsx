...
const Breadcrumb: React.FC<{ query?: ParsedUrlQuery }> = ({ query }) => {
  if (query) {
    ...
    if (Array.isArray(path)) {
      ...
      return (
        <ol className="no-scrollbar inline-flex flex-row-reverse items-center gap-1 overflow-x-scroll text-sm text-(--content-meta) md:gap-3">
          {path
            .slice(0)
            .reverse()
  ...
  return (
    <div className="text-sm text-(--content-meta) transition-all duration-75 hover:opacity-80">
      <HomeCrumb />
    </div>
  )
}
