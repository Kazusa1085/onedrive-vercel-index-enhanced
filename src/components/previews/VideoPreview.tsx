...
const VideoPlayer: FC<{
  ...
  useEffect(() => {
    ...
    axios
      .get(subtitle, { responseType: 'blob', signal: controller.signal })
      .then(resp => {
      ...
      .catch(err => {
        if (!axios.isCancel(err)) console.log('Could not load subtitle.')
      })

    if (isFlv && mpegts) {
      const loadFlv = () => {
        // Really hacky way to get the exposed video element from Plyr
        const video = document.getElementById('plyr') as HTMLMediaElement | null
        if (!video) return
...
const VideoPreview: FC<{ file: OdFileObject }> = ({ file }) => {
  ...
  const subtitle = buildRawUrl(vtt)

  // We also format the raw video file for the in-browser player as well as all other players
  const videoUrl = buildRawUrl(asPath)
    ...
    <>
      ...
      <DownloadBtnContainer>
        <div className="flex flex-wrap justify-center gap-2">
          ...
          <DownloadButton
            onClickCallback={() => {
              clipboard.copy(buildRawUrl(asPath))
